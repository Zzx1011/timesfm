...
def finetune(
    *,
    ...
    def process_train_batch(batch):
        ...
        pred_ts = tfm.predict(past_ts)  # 用 TimesFM 预测未来值
        delta_ts = actual_ts - pred_ts  # 计算误差 Δ(x_{t+1})
        return NestedMap(input_ts=past_ts, actual_ts=actual_ts, delta_ts=delta_ts)
    ...
    def train_step(states, prng_key, inputs):
        """训练步骤，仅训练 MMTimesFM"""
        timesfm_pred = tfm.predict(inputs.input_ts)  # TimesFM 预测未来值
        delta_target = inputs.actual_ts - timesfm_pred  # 计算真实误差 δ(x_{t+1})

        mm_pred_delta = mm_tfm.predict(inputs.input_ts, inputs.text_embedding)  # MMTimesFM 预测误差
        final_pred = timesfm_pred + mm_pred_delta  # 误差修正

        loss = jnp.mean(jnp.square(delta_target - mm_pred_delta))  # 仅优化 MMTimesFM，loss = MSE(真实误差, 预测误差)
        states = trainer_lib.update_model_state(jax_task, states, prng_key, loss)  # 更新 MMTimesFM 参数
        return states, loss

    def eval_step(states, prng_key, inputs):
        """评估步骤，测试 MMTimesFM 误差校正效果"""
        states = states.to_eval_state()
        timesfm_pred = tfm.predict(inputs.input_ts)  # TimesFM 预测
        mm_pred_delta = mm_tfm.predict(inputs.input_ts, inputs.text_embedding)  # MMTimesFM 预测误差
        final_pred = timesfm_pred + mm_pred_delta  # 误差修正

        loss = jnp.mean(jnp.square(inputs.actual_ts - final_pred))  # MSE(真实值, 修正后预测值)
        return states, loss
