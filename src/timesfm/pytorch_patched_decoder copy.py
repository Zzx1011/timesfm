...
class TextEmbedding(torch.nn.Module):
    ...
    def forward(self, text: torch.Tensor) -> torch.Tensor:
        """Generates text embeddings from a given text input.

        Args:
            text: A list of sentences or batch of sentences (List[str]).

        Returns:
            torch.Tensor: The embeddings for each input sentence.
        """
        # 使用BERT分词器将文本转化为token IDs
        ...
        # `outputs` 是一个包含多个值的元组：`last_hidden_state` 是我们关注的部分
        # 获取最后一个隐藏层的输出，形状是 [batch_size, seq_len, hidden_size]
        embeddings = outputs.last_hidden_state

        # 获取[CLS] token的嵌入（通常用于句子级别的嵌入）
        sentence_embeddings = embeddings[:, 0, :]  # [batch_size, hidden_size]
...
class PatchedTimeSeriesDecoder(nn.Module):
  ...
  def __init__(self, config: TimesFMConfig):
    ...
    if self.config.use_text_embedding:
      self.text_embedding = TextEmbedding(self.config.hidden_size)
      # Linear layer to project text embedding to the same shape as time-series input
  ...
  def _preprocess_input(
      ...
      """Preprocess input for stacked transformer with added text embedding."""
      ...
      if self.config.use_text_embedding:
        ...
        # Project the text embedding to match the dimensions
        text_emb = self.text_proj(text_emb)

        # Expand text_emb to match the batch size and sequence length
        text_emb = text_emb.unsqueeze(1).expand(-1, model_input.shape[1], -1)
        # Add text embedding to the model input
        model_input = model_input + text_emb
