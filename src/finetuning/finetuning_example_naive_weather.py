...
def plot_dataset() -> None:
    ...
    time_series, _ = get_time_series_data('p (mbar)')
   # 可视化时间序列数据
    ...
    plt.grid(True)
    plt.show()
    plt.savefig("weather_dataset.png")
    print("Weather dataset plot saved to weather_dataset.png")
    plt.close()
...
def single_gpu_example():
  ...
  context_len = 1024
  train_dataset, val_dataset = get_weather_dataset_data(context_len,
                                        tfm_config.horizon_len,
                                        freq_type=config.freq_type)
  print(f"Horizon length: {tfm_config.horizon_len}")
  ...
  plot_predictions(
      model=model,
      val_dataset=val_dataset,
      save_path=f"timesfm_predictions_{context_len}.png",
  )
...
def multi_gpu_example():
  ...
  train_dataset, val_dataset = get_weather_dataset_data(128, tfm_config.horizon_len)
  manager = mp.Manager()
